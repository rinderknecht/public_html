...
//
// NOTE: This version uses unsigned integers. It has been derived from
// mul2.c, which makes use of signed integers.
...
//
unsigned int lnz__ (const char bin[], unsigned int i) {
  return *bin == '0'? lnz__(bin+1,i+1) : i;
}

unsigned int lnz (const char bin[]) {
  return lnz__(bin,0);
}
...
// Multiplication
//
char* mul2u (const char p[], const char q[], unsigned int len) {
  char* r = NULL;
  ...
  if (zero_prefix (p,zpref_p)) {
    ...
    for (unsigned int i = 0; i < len; i++) r[i] = '0';

    unsigned int offset = 0; // Number of computed bits

    for (unsigned int j = len; !carry && lnz_q < j; j--) {
      // Computing bit #j-1
      if (q[j-1] == '1') {
        offset = len-j;
        for (unsigned int i = j; zpref_p < i+offset;) {
          i--;
