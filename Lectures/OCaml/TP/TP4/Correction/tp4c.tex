...
\title{Corrigé du TP 4 de programmation fonctionnelle en Objective Caml}
...
\begin{verbatim}
let rec préfixe a = match a with
  Vide -> []
...
\begin{verbatim}
let rec cherche x a = match a with
  Vide -> false
...
\begin{verbatim}
let rec ajout x a = match a with
...
\begin{verbatim}
let tri_liste l =
  infixe (List.fold_right ajout l Vide)
\end{verbatim}
...
\begin{verbatim}
let rec retire_minimum a = match a with
  Vide -> raise Not_found
| Noeud (x, Vide, droit) -> (x, droit)
