...
\begin{align*}
{\cal L} (\const{Const} \,\, \_) & = \varnothing\\
{\cal L} (\const{Var} \,\, x) & = \{x\}\\
{\cal L} (\const{BinOp} \, (\_,e_1,e_2)) & = {\cal L}(e_1) \cup {\cal
 L}(e_2)\\ 
{\cal L} (\const{Let} \, (x,e_1,e_2)) & = {\cal L}(e_1) \cup ({\cal
 L}(e_2) \backslash \{x\})\\
{\cal L} (\const{LetRec} \, (x,e_1,e_2)) & = ({\cal L}(e_1) \cup
   {\cal L}(e_2)) \backslash \{x\}\\
{\cal L} (\const{Fun} \, (x, e)) & = {\cal L} (e) \backslash \{x\}\\
{\cal L} (\const{App} \, (e_1,e_2)) & = {\cal L} (e_1) \cup {\cal L}
 (e_2)\\
{\cal L} (\const{If} \, (\_,e_1,e_2)) & = {\cal L} (e_1) \cup {\cal L}
 (e_2)\\
{\cal L} (\const{Ifz} \, (e_0,e_1,e_2)) & = {\cal L} (e_0) \cup {\cal L} (e_1) \cup {\cal L} (e_2)
\end{align*}
*)
